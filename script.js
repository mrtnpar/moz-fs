...
(() => {
    ...
    const DELIVERY_DAY_REG_EXP = /(delivery\s(Tomorrow|Mon|Tue|Wed|Thr|Fri|Sat|Sun),\s(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec).([0-9]?[0-9]))/;
    const CLEAN_UP_PRICES_REGEXP = /[\.\$,]/g;
    const CLEAN_UP_RATING_REGEXP = /[\.]/g;
    const cleanupProductCards = (productsCards) => {
    ...
    const extractProps = (productsCardsContent) => {
        ...
        return productsCardsContent.map((card) => {
            ...
            if (priceMatch && ratingMatch && deliveryMatch && url) {
                ...
                const delivery = deliveryMonth && deliveryDay
    ...
    const rank = (products) => {
        ...
        const minDelivery = Math.min(...products.map((product) => product.delivery));
        const maxRating = 5;
        ...
        return productsWithScore.sort((a, b) => b.score -
            a.score);
    };
    const allProductsCardsEls = document.querySelectorAll('[data-component-type="s-search-result"]');
    const getBestRates = (productsCardsElements, debug) => {
        const productsCardsElementsContent = cleanupProductCards(Array.from(productsCardsElements));
        const props = extractProps(productsCardsElementsContent.filter(Boolean));
        const ranked = rank(props.filter(Boolean));
        if (debug)
            console.table(ranked); // All products
        return ranked[0];
    };
